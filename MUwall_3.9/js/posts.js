    ...
    document.querySelectorAll('.action-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (action === 'fa-thumbs-up') {
                // 更新muwallPosts数组中的点赞数
                const postIndex = muwallPosts.findIndex(post => post.title === postTitle);
                if (postIndex !== -1) {
                    if (this.classList.contains('liked')) {
                        this.classList.remove('liked');
                        muwallPosts[postIndex].likes--;
                        countSpan.textContent = muwallPosts[postIndex].likes;
                    } else {
                ...
                if (confirm('确定要删除这条帖子吗？/ Are you sure you want to delete this post?')) {
                    const postIndex = muwallPosts.findIndex(post => post.title === postTitle);
                    if (postIndex !== -1) {
                        muwallPosts.splice(postIndex, 1);
                        saveMuwallPosts();
                        postElement.remove();
                    }
                }
    ...
    document.querySelectorAll('.comment-input button').forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (commentText) {
                ...
                // 2. 更新muwallPosts数组中的评论
                const postIndex = muwallPosts.findIndex(post => post.title === postTitle);
                if (postIndex !== -1) {
                    muwallPosts[postIndex].comments.push(newCommentObj);
                    saveMuwallPosts();
                }
                ...
                commentCountSpan.textContent = muwallPosts[postIndex].comments.length;
            ...
            if (title && content) {
                ...
                newPost.innerHTML = `
                    ...
                    <div class="post-actions">
                        ...
                        <button class="action-btn delete-btn" data-id="${newForumPost.id}"><i class="fas fa-trash"></i> <span data-i18n="post_delete">${translations[currentLang].post_delete}</span></button>
                    </div>
