...
function showView(viewId) {
    ...
    const currentIndex = views.indexOf(viewId);
    ...
    posts.forEach(post => {
        ...
        postElement.innerHTML = `
            ...
            <div class="post-actions">
                ...
                <button class="action-btn delete-btn" data-id="${post.id}"><i class="fas fa-trash"></i> <span data-i18n="post_delete">${translations[currentLang].post_delete}</span></button>
            </div>
...
function performSearch(keyword) {
    let searchResults = muwallPosts.filter(post => 
...
function loadProfilePosts() {
    ...
    let userPosts = muwallPosts.filter(post => post.author === currentUser);
    // 按时间戳降序排序，最新的帖子显示在最上方
